...
MIGRATION_NAME="${USER_MESSAGING__MIGRATION_NAME:-"InitialCreate"}"
CONTEXT_PROJECT_NAME="${USER_MESSAGING__CONTEXT_PROJECT_NAME:-"DataAccess"}"
STARTUP_PROJECT_NAME="${USER_MESSAGING__STARTUP_PROJECT_NAME:-"WebAPI"}"
ADD_MIGRATION="${USER_MESSAGING__ADD_MIGRATION:-"false"}"
APPLY_MIGRATION="${USER_MESSAGING__APPLY_MIGRATION:-"false"}"
...
if [[ "$ADD_MIGRATION" == "true" ]]; then
    ...
    dotnet ef migrations add "$MIGRATION_NAME" -p "/src/UserMessaging/$CONTEXT_PROJECT_NAME" -s "/src/UserMessaging/$STARTUP_PROJECT_NAME"
...
if [[ "$APPLY_MIGRATION" == "true" ]]; then
    ...
    dotnet ef database update "$MIGRATION_NAME" --project "/src/UserMessaging/$STARTUP_PROJECT_NAME"
