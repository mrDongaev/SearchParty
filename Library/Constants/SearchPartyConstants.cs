...
namespace Library.Constants
{
    public static class SearchPartyConstants
    {
        ...
        {
            MaxCount = EnvironmentUtils.TryGetEnvVariable("MAX_COUNT", out var maxCount) && int.TryParse(maxCount, out var maxCountInt) ? maxCountInt : 0;

            MinMmr = EnvironmentUtils.TryGetEnvVariable("MIN_MMR", out var minMmr) && int.TryParse(minMmr, out var minMmrInt) ? minMmrInt : 0;

            MaxMmr = EnvironmentUtils.TryGetEnvVariable("MAX_MMR", out var maxMmr) && int.TryParse(maxMmr, out var maxMmrInt) ? maxMmrInt : 20000;
