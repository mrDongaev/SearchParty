...
MIGRATION_NAME="${TEAM_PLAYER_PROFILES__MIGRATION_NAME:-"InitialCreate"}"
CONTEXT_PROJECT_NAME="${TEAM_PLAYER_PROFILES__CONTEXT_PROJECT_NAME:-"DataAccess"}"
STARTUP_PROJECT_NAME="${TEAM_PLAYER_PROFILES__STARTUP_PROJECT_NAME:-"WebAPI"}"
ADD_MIGRATION="${TEAM_PLAYER_PROFILES__ADD_MIGRATION:-"false"}"
APPLY_MIGRATION="${TEAM_PLAYER_PROFILES__APPLY_MIGRATION:-"false"}"
...
if [[ "$ADD_MIGRATION" == "true" ]]; then
    ...
    dotnet ef migrations add "$MIGRATION_NAME" -p "/src/TeamPlayerProfiles/$CONTEXT_PROJECT_NAME" -s "/src/TeamPlayerProfiles/$STARTUP_PROJECT_NAME"
...
if [[ "$APPLY_MIGRATION" == "true" ]]; then
    ...
    dotnet ef database update "$MIGRATION_NAME" --project "/src/TeamPlayerProfiles/$STARTUP_PROJECT_NAME"
